...
### 2. ✅ YouTube Embed Error 152/153
**Problem:**
- YouTube embeds showed "Error 152 Video player configuration error"
- Later showed "Error 153 Video player configuration error"
- "Watch video on YouTube" message instead of video
- Old markdown files had pre-existing iframe embeds from previous build system

**Root Cause:**
- Missing required iframe attributes in old iframe embeds
- Using youtube.com instead of youtube-nocookie.com
- Missing `allow` permissions
- Missing `title` attribute
- Old markdown posts contained iframes generated by previous JavaScript-based embed system
...
Added `_fix_old_youtube_iframes()` method to `RichEmbedProcessor` class that:
1. Detects old YouTube iframe embeds in markdown during build
2. Extracts video ID from old format
3. Replaces with properly formatted iframe with all required attributes
4. Processes these BEFORE converting plain URLs to embeds

**Why this works:**
- `youtube-nocookie.com` - Privacy-friendly, fewer restrictions
- `?rel=0` - Disables related videos at end
- `allow=` - Grants required permissions for embed playback
- `title=` - Accessibility compliance (WCAG)
- `loading="lazy"` - Performance optimization
- Handles both old iframe embeds AND plain YouTube URLs

**Verified:** Build process now correctly transforms old iframe embeds. Example from `2026_03_08_cope.md`:
- Markdown has: `<iframe src="https://www.youtube.com/embed/uwrHp_cxAvk">`
- Generated HTML has: `<iframe src="https://www.youtube-nocookie.com/embed/uwrHp_cxAvk?rel=0" allow="..." title="YouTube video">`
...
- [x] YouTube embeds work without Error 152/153 (verified with cope.html)
- [x] Old YouTube iframe embeds automatically upgraded during build
- [x] Plain YouTube URLs converted to proper embeds
- [x] Blog page shows only "← Home"
- [x] Blog posts show "← Blog"
- [x] Media upload shows file list with previews
